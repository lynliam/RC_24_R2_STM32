 ...
 *
 * 飞特舵机使用说明
 * 1. 创建飞特舵机对象，传入串口实例句柄  auto MotorInstance = ServoMotor(huart1);
 * 2. 注意波特率为 115200
 * 3. 然后直接调用对象方法即可
 *
 * 4. 注意飞特舵机的RS232板子驱动的定义为 Tx-Tx Rx-Rx
 * 方法概览:
 *
 *    设置电机ID：
 *        set_ID(0);
 *    发送目标速度和目标位置
 *        WritePosEx(int16_t Position, uint16_t Speed, uint8_t ACC);
 *    获取当前位置：
 *        get_current_speed()；
 *    获取当前位置：
 *        get_current_pos();
...
{
public:
    void Init(UART_HandleTypeDef uart_servo_handle_)
    {
